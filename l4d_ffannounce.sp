...
public Action:AnnounceFF(Handle:timer, any:attacker) 
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		...
		{
			CPrintToChat(attacker, "* {blue}你{default}对{olive}%N{default}造成了{olive}%d {green}伤害{default}！", DamageCache[attacker][i], i);			
			if (!IsFakeClient(i))
				CPrintToChat(i, "* {blue}%N{default}对{olive}你{default}造成了{olive}%d{green}{default}！", attacker, DamageCache[attacker][i]);				
			...
			if (GetConVarInt(AnnounceEnable) > 2)
			{
				if (IsClientObserver(i) || (GetClientTeam(i) == 3 && GetConVarInt(AnnounceEnable) == 3))
					...
					CPrintToChat(i, "* {blue}%N{default}对{olive}%N{default}造成了{olive}%d{green}伤害{default}！", attacker, DamageCache[attacker][i], i);					
			}
