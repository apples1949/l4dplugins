...
void ShowClientSkillMenu( int client )
{
	...
	Format(display, sizeof display, "积分: %.1f | 倍数: %.2f", Skills_GetClientMoney(client), Skills_GetClientMoneyMultiplier(client));
	menu.AddItem("", display, ITEMDRAW_DISABLED);
	
	//Format(display, sizeof display, "Team Money: %.1f | Multiplier: %.2f", Skills_GetTeamMoney(), Skills_GetTeamMoneyMultiplier());
	Format(display, sizeof display, "团队积分: %.1f | 倍数: %.2f", Skills_GetTeamMoney(), Skills_GetTeamMoneyMultiplier());
	menu.AddItem("", display, ITEMDRAW_DISABLED);
...
void ShowClientSkills( int client, int selection = 0 )
{
	...
	Format(name, sizeof name, "购买的技能数量: %i", Skills_GetClientSkillsCount(client));
