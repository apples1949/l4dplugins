...
public void OnPluginStart() {
	...
	Enabled = 			CreateConVar("headshot_buff_enabled", "1", "是否启用爆头击杀带来的增益", FCVAR_NOTIFY);
	Duration_hurt =		CreateConVar("headshot_buff_gain_hurt", "0.3", "击中头部的增益持续多长时间 0:仅播放声音", FCVAR_NOTIFY);
	Duration_kill =		CreateConVar("headshot_buff_gain_kill", "0.6", "爆头击杀的增益持续多长时间 0:仅播放声音", FCVAR_NOTIFY);
	Keep_switch =		CreateConVar("headshot_buff_keep", "0", "切换武器时是否保持增益 0:清除 1:保持", FCVAR_NOTIFY);
	Sound_hurt =		CreateConVar("headshot_buff_sound_hurt", "ui/littlereward.wav", "击中头部播放什么声音 空:无", FCVAR_NOTIFY);
	Sound_kill =		CreateConVar("headshot_buff_sound_kill", "level/bell_normal.wav", "爆头击杀播放什么声音 空:无", FCVAR_NOTIFY);
	Death_only =		CreateConVar("headshot_buff_death", "0", "death only, ignore headshot hurt", FCVAR_NOTIFY);
	Speed_rate =		CreateConVar("headshot_buff_speed", "1.5", "爆头增加几倍速度", FCVAR_NOTIFY);
	Buff_actions =		CreateConVar("headshot_buff_actions", "-1", "增强什么动作 1=射击 2=部署'弹药包' 4=换弹 8=使用近战", FCVAR_NOTIFY);
