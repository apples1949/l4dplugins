...
public void OnPluginStart()
{
	...
	l4d_flying_car_color = CreateConVar("l4d_flying_car_color", "", "自定义颜色(RGB)，留下黑色以使用默认颜色", 0);
	l4d_flying_car_enable = CreateConVar("l4d_flying_car_enable", "1", "启用/禁用插件. 0:禁用, 1:启用", 0, true, 0.0, true, 1.0);
	l4d_flying_car_explode = CreateConVar("l4d_flying_car_explode", "1", "汽车爆炸? 0:禁用, 1:启用", 0, true, 0.0, true, 1.0);
	l4d_flying_car_ignite = CreateConVar("l4d_flying_car_ignite", "1", "离开时点燃汽车? 0:禁用, 1:启用", 0, true, 0.0, true, 1.0);
	l4d_flying_car_model = CreateConVar("l4d_flying_car_model", "1", "汽车模型 (1:出租车, 2:警车)", 0, true, 1.0, true, 2.0);
	l4d_flying_car_random_color = CreateConVar("l4d_flying_car_random_color", "1", "随机选择颜色，而不是使用自定义颜色? 0:禁用, 1:启用", 0, true, 0.0, true, 1.0);
	l4d_flying_car_random_model = CreateConVar("l4d_flying_car_random_model", "1", "随机选择模型，而不是使用自定义颜色? 0:禁用, 1:启用", 0, true, 0.0, true, 1.0);
...
public void event_finale_vehicle_leaving(Handle event, const char[] name, bool dontBroadcast)
{
	...
	PrintToChatAll("\x04Harry Potter(插件作者): \x03It's time to Hogwarts School !!\n(是时候前往霍格沃兹学院了!!)");
