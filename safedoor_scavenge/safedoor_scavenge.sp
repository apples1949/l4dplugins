...
public void OnPluginStart()
{
	...
	g_hNumCansNeeded = CreateConVar("safedoor_scavenge_needed", "8", "一般情况下，需要添加多少桶油才能解锁安全室的门", _, true, 0.0);
	g_hMinTravelDistance = CreateConVar("safedoor_scavenge_min_dist", "250.0", "刷新的油桶位置和地标之间的最小距离", _, true, 0.0);
	g_hMaxTravelDistance = CreateConVar("safedoor_scavenge_max_dist", "2500.0", "刷新的油桶位置和地标之间的最大距离", _, true, 0.0);
	g_hCansNeededPerPlayer = CreateConVar("safedoor_scavenge_per_player", "1.5", "每个玩家需要多少桶油才能打开安全室的门(大于0的值将覆盖safedoor_scavenge_needed的值。0=使用默认设置)", _, true, 0.0);
	g_hAllowMultipleFill = CreateConVar("safedoor_scavenge_multiple_fill", "0", "允许同时灌多少油？");
	g_hScavengePanicTime = CreateConVar("safedoor_scavenge_panic_time", "30.0", "灌油事件开始后多长时间召唤尸潮(0.0=off)", _, true, 0.0);
