...
{	
	...
	g_cvarPhysics 		= CreateConVar("l4d2_spawn_props_physics", 				"1", "是否启用菜单中的物理对象", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvarDynamic 		= CreateConVar("l4d2_spawn_props_dynamic",				"1", "是否启用菜单中的动态（非实体）物体", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvarStatic 		= CreateConVar("l4d2_spawn_props_static",				"1", "是否 启用菜单中的静态（固体）物体", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvarItem 			= CreateConVar("l4d2_spawn_props_items",				"1", "是否启用菜单中的物品和武器对象", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvarVehicles 		= CreateConVar("l4d2_spawn_props_category_vehicles",	"1", "是否启用'车辆'类别", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvarFoliage 		= CreateConVar("l4d2_spawn_props_category_foliage",		"1", "是否启用'植物'类别", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvarInterior 		= CreateConVar("l4d2_spawn_props_category_interior",	"1", "是否启用'室内'类别", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvarExterior 		= CreateConVar("l4d2_spawn_props_category_exterior",	"1", "是否启用'外观'类别", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvarDecorative 	= CreateConVar("l4d2_spawn_props_category_decorative",	"1", "是否启用'装饰'类别", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvarMisc 			= CreateConVar("l4d2_spawn_props_category_misc", 		"1", "是否启用'杂项'类别", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_cvarLog 			= CreateConVar("l4d2_spawn_props_log_actions", 			"0", "管理员产生对象时是否记录到日志?", CVAR_FLAGS, true, 0.0, true, 1.0);
...
public Action CmdDebugProp(int client, int args)
{
	...
	{
		CPrintToChat(client, "%T","You are not looking at a valid object", client);
	...
	CPrintToChat(client, "%T", "Object Model", client, Object, m_ModelName);

	static char name[256];
	GetEntPropString(Object, Prop_Data, "m_iName", name, sizeof(name));
	CPrintToChat(client, "%T", "Object Targetname", client, name);

	static float position[3];
	GetEntPropVector(Object, Prop_Send, "m_vecOrigin", position);
	CPrintToChat(client, "%T", "Object Position", client, position[0], position[1], position[2]);

	static float angle[3];
	GetEntPropVector(Object, Prop_Data, "m_angRotation", angle);
	CPrintToChat(client, "%T", "Object Angle", client, angle[0], angle[1], angle[2]);
...
public Action CmdSpawnProp(int client, int args)
{
	...
	if(args < 3)
	{
		CPrintToChat(client, "Usage: sm_spawnprop <model> [static | dynamic | physics] [cursor | origin]");
		return Plugin_Handled;
	}
	char arg1[256];
	char arg2[256];
	...
	if(!IsModelPrecached(model))
	{
		...
		{
			CPrintToChat(client, "There was a problem spawning the selected model [ERROR: Invalid Model]");
			return Plugin_Handled;
		}
	}
	...
	if(strcmp(arg2, "static", false) == 0)
	{
		...
		if(strcmp(arg3, "cursor") == 0)
		{
			...
			{
				CPrintToChat(client, "Vector out of world geometry. Spawning on current position instead");
			}
		}
		...
		{
			CPrintToChat(client, "Invalid spawn method specified. Use: [cursor | origin]");
			return Plugin_Handled;
		}
		VecAngles[0] = 0.0;
		VecAngles[2] = 0.0;
	...
	{
		CPrintToChat(client, "Invalid render mode. Use: [static | dynamic | physics]");
		return Plugin_Handled;
	}
	return Plugin_Handled;
}
...
public int MenuHandler_DA_Ask(Menu menu, MenuAction action, int param1, int param2)
{
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			{
				DeleteAllProps();
				CPrintToChat(param1, "%T", "Successfully deleted all spawned objects", param1);
			}
			else
			{
				CPrintToChat(param1, "%T", "Canceled", param1);
			}
	...
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			if(strcmp(menucmd, "sm_spdeleteall")== 0)
			{
				...
				CPrintToChat(param1, "%T", "delete all the spawned objects?", param1);
			}
	...
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			if(g_iCategory[param1] == 1)
			{
				...
				{
					CPrintToChat(param1, "Vector out of world geometry. Spawning on current position instead");
				}
				VecAngles[0] = 0.0;
				VecAngles[2] = 0.0;
				DispatchKeyValueVector(prop, "angles", VecAngles);
		...
		case MenuAction_Select:
		{
			...
			switch(g_iMoveCategory[param1])
			{
				case 1:
				{
					...
					if(Object <= MaxClients || !IsValidEntity(Object))
					{
						CPrintToChat(param1, "%T","You haven't locked anything yet", param1);
...
void DeleteLookingEntity(int client)
{
	...
	CPrintToChat(client, "%T", "Successfully removed an object", client, Object);
...
void DeleteLastProp(int client)
{
	...
	{
		CPrintToChat(client, "%T", "You haven't spawned anything yet", client);
		return;
	}
	...
	if(Object > MaxClients && IsValidEntity(Object))
	{
		...
		|| strcmp(class, "upgrade_laser_sight", false) == 0)
		{
			...
			CPrintToChat(client, "%T","Succesfully deleted the last spawned object",client);
			g_iLastObject[client] = -1;
			g_bSpawned[Object] = false;
			g_bUnsolid[Object] = false;
		...
		{
			CPrintToChat(client, "%T", "The last spawned object index is not an object anymore!", client, Object);
			g_iLastObject[client] = -1;
			g_bSpawned[Object] = false;
			g_bUnsolid[Object] = false;
		}
	...
	{
		CPrintToChat(client, "%T","The last object is not valid anymore",client);
	}
}
...
void SaveMapStripper(int client)
{
	...
	if(file == null)
	{
		CPrintToChat(client, "Failed to create or overwrite the map file");
		CPrintToChat(client, "{green}Failed to create or overwrite the map file");
		PrintHintText(client, "Failed to create or overwrite the map file");
		PrintToConsole(client, "Failed to create or overwrite the map file");
		PrintCenterText(client, "Failed to create or overwrite the map file");
		return;
	}
	
	CPrintToChat(client, "{green}%T", "Saving the content. Please Wait", client);
	...
	FlushFile(file);
	CloseHandle(file);
	CPrintToChat(client, "{lightgreen}%T (addons/stripper/maps/%s.cfg)", "Succesfully saved the map data", client, map);
}
...
public Action CmdRotate(int client, int args)
{
	...
	if(args < 2)
	{
		CPrintToChat(client, "Usage: sm_prop_rotate <axys> <angles> [EX: !prop_rotate x 30]");
		return Plugin_Handled;
	}
	//int Object = g_iLastObject[client];
	...
	if(Object <= MaxClients || !IsValidEntity(Object))
	{
		CPrintToChat(client, "%T","You are not looking at a spawned object", client);
		return Plugin_Handled;
	}
	...
	{
		CPrintToChat(client, "Invalid Axys (x,y,z are allowed)");
	}
	TeleportEntity(Object, NULL_VECTOR, vecAngles, NULL_VECTOR);
...
public Action CmdRemoveAll(int client, int args)
{
	...
	CPrintToChat(client, "{green}%T","Are you sure(Delete All)?",client);
...
public Action CmdMove(int client, int args)
{
	...
	if(args < 2)
	{
		CPrintToChat(client, "Usage: sm_prop_move <axys> <distance> [EX: !prop_move x 30]");
		return Plugin_Handled;
	}
...
public Action CmdSetAngles(int client, int args)
{
	...
	if(args < 3)
	{
		CPrintToChat(client, "Usage: sm_prop_setang <X Y Z> [EX: !prop_setang 30 0 34]");
		return Plugin_Handled;
	}
...
public Action CmdSetPosition(int client, int args)
{
	...
	if(args < 3)
	{
		CPrintToChat(client, "Usage: sm_prop_setpos <X Y Z> [EX: !prop_setpos 505 -34 17");
		return Plugin_Handled;
	}
...
public Action CmdLock(int client, int args)
{
	...
	CPrintToChat(client, "%T", "Succesfully locked spawned object", client, Object);
...
public Action CmdClone(int client, int args)
{
	...
	if(Object <= MaxClients || !IsValidEntity(Object))
	{
		CPrintToChat(client, "%T", "You haven't locked anything yet", client);
		return Plugin_Handled;
	}
