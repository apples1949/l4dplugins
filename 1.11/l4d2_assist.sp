...
public void OnPluginStart()
{
	...
	g_hCvarAllow = 		CreateConVar(	"sm_assist_enable", 		"1", 			"如果为1,启用插件.", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_hCvarModes =		CreateConVar(	"sm_assist_modes",			"",				"什么模式启用插件,用逗号隔开(无空格). (无内容 = 全模式).", CVAR_FLAGS );
	g_hCvarModesOff =	CreateConVar(	"sm_assist_modes_off",		"",				"什么模式禁用插件,用逗号隔开(无空格). (无内容 = 无)", CVAR_FLAGS );
	g_hCvarModesTog =	CreateConVar(	"sm_assist_modes_tog",		"0",			"什么模式启用插件. 0=全部模式, 1=战役, 2=生还者, 4=对抗, 8=清道夫. 数字相加在一起.", CVAR_FLAGS );
	g_hTankOnly = 		CreateConVar(	"sm_assist_tank_only", 		"0", 			"如果为1, 只显示坦克伤害.",CVAR_FLAGS, true, 0.0, true, 1.0);
...
public void Event_PlayerIncapacitated(Event event, const char[] name, bool dontBroadcast) 
{
	...
	CPrintToChatAll("[{olive}VS{default}] %t", "Witch_Health", health);
	CPrintToChatAll("[{olive}VS{default}] %t", "Witch_Incap", victim);
...
public void Event_Player_Death(Event event, const char[] name, bool dontBroadcast) 
{
	...
	if (attacker == 0)
	{
		...
		if (g_bIsWitch[witchid] && g_bShouldAnnounceWitchDamage[witchid])  // Prevent double print on witch incapping 2 players (rare)
		{
			...
			CPrintToChatAll("{default}[{olive}VS{default}] %t", "Witch_Health", health);
			CPrintToChatAll("{default}[{olive}VS{default}] %t", "Witch_Kill", victim);
			g_bShouldAnnounceWitchDamage[witchid] = false;
			return;
		}
	}
	...
	if (attacker && IsClientInGame(attacker) && GetClientTeam(attacker) == L4D_TEAM_SURVIVOR && GetClientTeam(victim) == L4D_TEAM_INFECTED)
	{
		...
		CPrintToChatAll("[{olive}VS{default}] %t", "Got_Killed_By", victim, attacker, g_iDamage[attacker][victim]);
		if (AssistFlag == true) 
		{
