...
stock void HandlePop(int attacker, int victim, int shoveCount, float timeAlive)
{
	...
	if (GetConVarBool(g_hCvarReport) && GetConVarBool(g_hCvarReportOld))
	{
		...
		{
            CPrintToChatAll( "{olive}%N{default}悄无声息解决了 {olive}%N{default}", attacker, victim );
		}
		else if (IS_VALID_INGAME(attacker))
		{
            CPrintToChatAll( "{olive}%N{default}悄无声息解决了一只Boomer", attacker );
		}
    }
...
stock void HandleShove(int attacker, int victim, int zombieClass)
{
	...
	if (GetConVarBool(g_hCvarReport) && GetConVarBool(g_hCvarReportOld))
	{
		if (IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim))
		{
            CPrintToChatAll( "{olive} %N {blue}推到了{olive} %N {default}", attacker, victim );
		}
		else if (IS_VALID_INGAME(attacker))
		{
            CPrintToChatAll( "{olive} %N {blue}推到了{default} 一个 {olive}特感{default}.", attacker );
		}
    }
...
stock void HandleSkeet(int attacker, int victim, bool bMelee = false, bool bSniper = false, bool bGL = false)
{
	...
	if (GetConVarBool(g_hCvarReport) && GetConVarBool(g_hCvarReportOld))
	{
		if (attacker == -2)
		{
			...
			{
				CPrintToChatAll("{green}★★{olive} %N {default} 被 {blue}团队空爆{default}", victim);
			}
			else {
				CPrintToChatAll("{green}★★{olive} Hunter {default} 被 {blue}团队空爆{default}");
			}
		}
		else if (IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim))
		{
			CPrintToChatAll("{green}★★{olive} %N {blue} %s空爆{default} 了 {olive} %N {default}", attacker, (bMelee) ? "近战" : ((bSniper) ? "爆头" : ((bGL) ? "榴弹" : "")), victim);
		}
		else if (IS_VALID_INGAME(attacker))
		{
			CPrintToChatAll("{green}★{olive} %N {blue} %s空爆{default} 了 {olive}一只hunter{default}", attacker, (bMelee) ? "近战" : ((bSniper) ? "爆头" : ((bGL) ? "榴弹" : "")));
		}
    }
...
void HandleCrown(int attacker, int damage)
{
	...
	if (GetConVarBool(g_hCvarReport) && GetConVarBool(g_hCvarReportOld))
	{
		...
		{
			CPrintToChatAll("{green}★★★{olive} %N {default} 秒了Witch ({blue} %i {default} 伤害)", attacker, damage);
		}
		else {
			CPrintToChatAll("Witch {blue}被秒杀了{default}");
		}
	}
...
void HandleDrawCrown(int attacker, int damage, int chipdamage)
{
	...
	if (GetConVarBool(g_hCvarReport) && GetConVarBool(g_hCvarReportOld))
	{
		if (IS_VALID_INGAME(attacker))
		{
			CPrintToChatAll("{green}★★{olive} %N {default} 引秒了一个Witch ({blue} %i {default} 伤害, {olive} %i {default} 引秒伤害)", attacker, damage, chipdamage);
		}
		else {
			CPrintToChatAll("Witch 被引秒 ({blue} %i {default} 伤害, {olive} %i {default} 引秒伤害)", damage, chipdamage);
		}
    }
...
void HandleTongueCut(int attacker, int victim)
{
	...
	if (GetConVarBool(g_hCvarReport))
	{
		if (IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim))
		{
			CPrintToChatAll("{green}★★★{olive} %N {blue}砍断{default} {olive} %N {default}的舌头", attacker, victim);
		}
		else if (IS_VALID_INGAME(attacker))
		{
			CPrintToChatAll("{green}★★★{olive} %N {blue}砍断{default} smoker 的舌头", attacker);
		}
    }
...
void HandleSmokerSelfClear(int attacker, int victim, bool withShove = false)
{
	...
	if (GetConVarBool(g_hCvarReport))
	{
		if (IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim))
		{
            CPrintToChatAll( "{green}★★{olive} %N {default}被{olive}%N{default}的舌头拉后{blue}%s自救", attacker, victim, (withShove) ? "推停" : "" );
		}
		else if (IS_VALID_INGAME(attacker))
		{
            CPrintToChatAll( "{green}★★{olive} %N {default}被{olive}Smoker{default}的舌头拉后{blue}%s自救", attacker, (withShove) ? "推停" : "" );
		}
	}
	// call forward
...
stock void HandleBHopStreak(int survivor, int streak, float maxVelocity)
{
	...
	{
        CPrintToChat(survivor, "{green}★{olive} 你 {default}连跳了{blue} %i {default}次 ({blue}最高速度: {default} %.1f)",
                streak,
                maxVelocity
            );
	}
