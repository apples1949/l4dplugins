...
public void OnPluginStart()
{
	...
	RegServerCmd("l4d_wlimits_add", AddLimit_Cmd, "添加一条武器限制规则");
	RegServerCmd("l4d_wlimits_lock", LockLimits_Cmd, "锁定武器限制规则以提高搜索速度");
	RegServerCmd("l4d_wlimits_clear", ClearLimits_Cmd, "清除所有的武器限制规则(限制规则必须被锁定才能被清除)");
...
public Action ClearLimits_Cmd(int args)
{
	if (bIsLocked) {
		bIsLocked = false;
		PrintToChatAll("[L4D Weapon Limits]武器限制规则已清除!");
		ClearLimits();
	}
...
void denyWeapon(int wep_slot, LimitArrayEntry arrayEntry, int client)
{
	...
	if (bIsPressingButtonUse[client] && !bIsHoldingButtonUse[client])
	{
		...
		CPrintToChat(client, "{blue}[{default}Weapon Limits{blue}]{default} 此武器最多只能获取{green} %d {default}把", arrayEntry.LAE_iLimit);
