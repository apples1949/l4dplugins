...
public void OnPluginStart() 
{
	...
	g_cvarWelcome = CreateConVar("sm_helpmenu_welcome", "1", "向新连接的用户显示欢迎信息", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_cvarAdmins = CreateConVar("sm_helpmenu_admins", "1", "在菜单中显示在线管理员的名单", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_cvarRotation = CreateConVar("sm_helpmenu_rotation", "0", "在菜单中显示地图轮换", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_cvarReload = CreateConVar("sm_helpmenu_autoreload", "1", "更换地图时自动重新加载配置文件", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_cvarConfigPath = CreateConVar("sm_helpmenu_config_path", "configs/helpmenu.cfg", "配置文件的路径");
	g_cvarSteamGroup = CreateConVar("sm_helpmenu_steam_group", "0", "在菜单中显示'加入我们的Steam组'项目", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_cvarDoNotDisplay = CreateConVar("sm_helpmenu_do_not_display", "1", "在菜单中显示 '不再显示'和'下次再显示项目'", FCVAR_NOTIFY, true, 0.0, true, 1.0);
