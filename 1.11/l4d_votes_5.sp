...
{	
	...
	{
		CPrintToChatAll("{default}[{olive}VOTE{default}]地图将在{green}%d{default}秒后重置", READY_RESTART_MAP_DELAY+1);		
		RestartMapDelayed();
	}
...
{ 
	...
	{
		CPrintToChat(client, "{default}[{olive}VOTE{default}]投票菜单插件已关闭!");
	}
		...
		switch (itemNum)
		{
			case 1: 
			{
				if (VotensHpE_D == false)
				{
					...
					CPrintToChat(client, "{default}[{olive}VOTE{default}]回血已禁用");
				}
			...
			case 2: 
			{
				...
				{
					FakeClientCommand(client,"sm_votes");
					CPrintToChat(client, "{default}[{olive}VOTE{default}]开启全体语音已禁用");
				}
			...
			case 5: 
			{
				...
				{
					FakeClientCommand(client,"sm_votes");
					CPrintToChat(client, "[{olive}VOTE{default}]投票更换官图已禁用,请使用!votemap");
				}
...
public Action Command_VoteHp(int client, int args)
{
	...
	&& VotensHpE_D == true)
	{
		...
		{
			CPrintToChatAll("[{olive}VOTE{default}]{olive} %N {default}发起了一个投票: {blue}全体回血",client);
...
public void DisplayVoteKickMenu(int client)
{
	...
	if(CanStartVotes(client))
	{
		...
		CPrintToChatAll("[{olive}VOTE{default}]{olive} %N {default}发起投票: {blue}踢出 %s", client, kickplayer_name);
...
public Action Command_VotemapsMenu(int client, int args)
{
	...
	{
		CPrintToChat(client, "[{olive}VOTE{default}]官图投票已禁止,请使用!votemap投票");
	}
	return Plugin_Handled;
}
...
public Action Command_Votemaps2Menu(int client, int args)
{
	...
	{
		CPrintToChat(client, "[{olive}VOTE{default}]三方图投票已禁止,请使用!votemap");
	}
	return Plugin_Handled;
}
...
public void DisplayVoteMapsMenu(int client)
{
	...
	if(CanStartVotes(client))
	{
		...
		CPrintToChatAll("[{olive}VOTE{default}]{olive} %N {default}发起投票: {blue}更换地图%s", client, votesmapsname);
...
public int Handler_VoteCallback(Menu menu, MenuAction action, int param1, int param2)
{
	...
	else if (action == MenuAction_VoteEnd)
	{
		...
		else
		{
			...
			CPrintToChatAll("[{olive}VOTE{default}]{blue}投票通过。{default}(同意：{green}%d%%%%{default}, 不同意：{green}%i{default})", RoundToNearest(100.0*percent), totalVotes);
			CreateTimer(2.0, VoteEndDelay);
...
public Action Timer_forcespectate(Handle timer, any client)
{
	...
	if (g_iSpectatePenaltyCounter[client] != 0)
	{
		if (GetClientTeam(client) == 3||GetClientTeam(client) == 2)
		{
			...
			CPrintToChat(client, "[{olive}VOTE{default}]你被投票强制旁观!请在{green}%d秒{default}后输入!join回到队伍", g_iSpectatePenaltyCounter[client]);
	...
	else if (g_iSpectatePenaltyCounter[client] == 0)
	{
		...
		{
			CPrintToChat(client, "[{olive}VOTE{default}]你现在可以通过输入!join回到队伍了");	//only print this hint text to the spectator if he tried to join the infected team, and got swapped before
		}
...
bool TestVoteDelay(int client)
{
 	...
 	if (delay > 0)
 	{
 		if (delay > 60)
 		{
 			CPrintToChat(client, "[{olive}VOTE{default}]你必须等待{red}%i秒{default}后再发起投票!", delay % 60);
 		}
 		else
 		{
 			CPrintToChat(client, "[{olive}VOTE{default}]你必须等待{red}%i秒{default}后再发起投票!", delay);
 		}
 		return false;
 	}
