...
public Action Command_VoteHp(int client, int args)
{
	...
	&& VotensHpE_D == true)
	{
		...
		{
			CPrintToChatAll("{default}{olive} %N {default}发起了一个投票: {blue}全体回血",client);
...
public Action Command_VoteAlltalk(int client, int args)
{
	...
	&& VotensAlltalkE_D == true)
	{
		...
		if(CanStartVotes(client))
		{
			CPrintToChatAll("{default}{olive}%N{default}发起了一个投票: {blue}开启全体语音",client);
...
public Action Command_VoteAlltalk2(int client, int args)
{
	...
	&& VotensAlltalk2E_D == true )
	{
		...
		if(CanStartVotes(client))
		{
			CPrintToChatAll("{default}{olive} %N {default}发起投票: {blue}关闭全体语音",client);
...
public Action Command_VoteRestartmap(int client, int args)
{
	...
	&& VotensRestartmapE_D == true)
	{
		...
		if(CanStartVotes(client))
		{
			CPrintToChatAll("{default}{olive} %N {default}发起了一个投票: {blue}重置当前地图",client);
...
public int Menu_VotesKick(Menu menu, MenuAction action, int param1, int param2)
{
	if (action == MenuAction_Select)
	{
		...
		if(player && IsClientInGame(player))
		{
			if (player == param1)
			{
				CPrintToChatAll("你确定要踢你自己吗?请重新选择");
			...
			{
				CPrintToChat(param1, "该玩家无法被踢出，请重新选择你想踢出的玩家!");
				CPrintToChat(player, "{default}{olive}%N{default}尝试投票将你踢出, 但你拥有权限无法踢出", param1);
				CreateVoteKickMenu(param1);
			}
			else
			{
		...
		{
			CPrintToChatAll("{default}该玩家已不在游戏中, 请重新选择!");
			CreateVoteKickMenu(param1);
		}	
	}
	else if ( action == MenuAction_Cancel)
...
public void DisplayVoteKickMenu(int client)
{
	...
	if(CanStartVotes(client))
	{
		...
		CPrintToChatAll("{default}{olive} %N {default}发起投票: {blue}踢出 %s", client, kickplayer_name);
...
public void DisplayVoteMapsMenu(int client)
{
	...
	if(CanStartVotes(client))
	{
		...
		CPrintToChatAll("{default}{olive} %N {default}发起投票: {blue}更换地图%s", client, votesmapsname);
...
public Action Command_Votesforcespectate(int client, int args)
{
	...
	{
		CPrintToChat(client, "强制玩家旁观已被禁止");
	}
	return Plugin_Handled;
}
...
public void DisplayVoteforcespectateMenu(int client)
{
	...
	if(CanStartVotes(client))
	{
		...
		CPrintToChatAll("{default}{olive} %N {default}发起投票: {blue}强制玩家%s旁观{default}, 只有投票发起者的阵营能查看投票进度", client, forcespectateplayername);
...
public Action Timer_forcespectate(Handle timer, any client)
{
	...
	else if (g_iSpectatePenaltyCounter[client] == 0)
	{
		...
		{
			CPrintToChat(client, "你现在可以回到队伍了");	//only print this hint text to the spectator if he tried to join the infected team, and got swapped before
		}
...
bool TestVoteDelay(int client)
{
 	...
 	if (delay > 0)
 	{
 		if (delay > 60)
 		{
 			CPrintToChat(client, "你必须等待{red}%i秒{default}后再发起投票!", delay % 60);
 		}
 		else
 		{
 			CPrintToChat(client, "你必须等待{red}%i秒{default}后再发起投票!", delay);
 		}
 		return false;
 	}
...
bool CanStartVotes(int client)
{
	...
	{
		CPrintToChat(client, "无法发起投票。需要{red}%d{default}个玩家.", g_iCvarPlayerLimit);
		return false;
	}
	return true;
}
...
public Action COLD_DOWN(Handle timer,any client)
{
	switch (g_voteType)
	{
		...
		case (view_as<voteType>(kick)):
		{
			...
			CPrintToChatAll("%t","votes3_15", kickplayer_name);
		...
		case (view_as<voteType>(forcespectate)):
		{
			...
			{
				CPrintToChatAll("玩家{blue}%s{default}已被强制旁观!", forcespectateplayername);
			...
			{
				CPrintToChatAll("无法找到玩家%s", forcespectateplayername);	
			}
		}
	}
