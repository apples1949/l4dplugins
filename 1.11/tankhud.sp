...
public void OnPluginStart()
{
	...
	g_hPanelTitle = CreateConVar("tank_hud_title", "Tank Hud", "Tank 状态面板标题", CVAR_FLAG);
...
public Action Cmd_TankHud(int client, int args)
{
	if (!hiddenPanel[client])
	{
		...
		CPrintToChat(client, "{green}<TankHUD>：{default}Tank HUD已{lightgreen}关闭");
		return Plugin_Continue;
	}
	hiddenPanel[client] = false;
	CPrintToChat(client, "{green}<TankHUD>：{default}Tank HUD已{lightgreen}开启");
	return Plugin_Continue;
}
...
public void evt_TankSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	for (i = 1; i <= MaxClients; i++)
	{
		...
		CPrintToChat(i, "{green}<TankHUD>：{default}TankHUD 已{lightgreen}关闭\n聊天框输入 {olive}!tankhud {default}开启{lightgreen}tankhud");
	}
}
