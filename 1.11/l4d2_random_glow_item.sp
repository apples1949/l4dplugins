...
public void OnPluginStart()
{
    ...
    g_hCvar_Enabled        = CreateConVar("l4d2_random_glow_item_enable", "1", "是否启用插件.\n0 = 禁用, 1 = 启用.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_RemoveSpawner  = CreateConVar("l4d2_random_glow_item_remove_spawner", "1", "当*_spawn实体的数量达到0时删除.\n0 = 禁用, 1 = 启用.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_HealthCabinet  = CreateConVar("l4d2_random_glow_item_health_cabinet", "1", "药柜被打开后移除发光.\n0 = 禁用, 1 = 启用.", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_MinBrightness  = CreateConVar("l4d2_random_glow_item_min_brightness", "0.5", "检测随机颜色的辉光最小亮度的算法值(不准确).", CVAR_FLAGS, true, 0.0, true, 1.0);
    g_hCvar_ScavengeGascan = CreateConVar("l4d2_random_glow_item_scavenge_gascan", "0", "需要收集的油桶是否由插件发光.\n0 = 禁用, 1 = 启用.", CVAR_FLAGS, true, 0.0, true, 1.0);
    ...
    // Admin Commands
    RegAdminCmd("sm_glowinfo", CmdInfo, ADMFLAG_ROOT, "向聊天输出有关当前准心处实体的发光信息.");
    RegAdminCmd("sm_glowreload", CmdReload, ADMFLAG_ROOT, "重新加载发光插件配置.");
    RegAdminCmd("sm_glowremove", CmdRemove, ADMFLAG_ROOT, "从准心处的实体移除插件发光.");
    RegAdminCmd("sm_glowremoveall", CmdRemoveAll, ADMFLAG_ROOT, "删除插件创建的所有发光.");
    RegAdminCmd("sm_glowadd", CmdAdd, ADMFLAG_ROOT, "将发光(使用默认配置)添加到准心处的实体.");
    RegAdminCmd("sm_glowall", CmdAll, ADMFLAG_ROOT, "将发光(使用默认配置)添加到所有可能的地方.");
    RegAdminCmd("sm_print_cvars_l4d2_random_glow_item", CmdPrintCvars, ADMFLAG_ROOT, "将插件相关的 cvar 及其各自的值打印到控制台.");
}
...
Action CmdInfo(int client, int args)
{
    ...
    if (entity == -1)
    {
        PrintToChat(client, "\x04无效目标");
        return Plugin_Handled;
    }

    if (!HasEntProp(entity, Prop_Send, "m_glowColorOverride"))
    {
        PrintToChat(client, "\x04目标实体没有发光属性.");
        return Plugin_Handled;
    }
    ...
    PrintToChat(client, "\x05索引: \x03%i \x05类名: \x03%s \x05模型: \x03%s \x05发光颜色 (RGB值|整数): \x03%i %i %i|%i \x05亮度: \x03%.1f \x05类型: \x03%i \x05显示: \x03%i \x0范围(最小|最大): \x03%i|%i", entity, classname, modelname, rgb[0], rgb[1], rgb[2], color, GetRGB_Brightness(rgb), glowType, glowFlashing, glowRangeMin, glowRangeMax);
...
Action CmdReload(int client, int args)
{
    ...
    if (IsValidClient(client))
        PrintToChat(client, "\x04已重载发光插件配置");
...
Action CmdRemove(int client, int args)
{
    ...
    PrintToChat(client, "\x04已移除目标实体的插件发光");
...
Action CmdRemoveAll(int client, int args)
{
    ...
    if (IsValidClient(client))
        PrintToChat(client, "\x04已移除所有的插件发光");
...
Action CmdAdd(int client, int args)
{
    ...
    PrintToChat(client, "\x04已为目标实体添加插件发光");
...
Action CmdAll(int client, int args)
{
    ...
    if (IsValidClient(client))
        PrintToChat(client, "\x04已为所有有效实体添加插件发光");
