...
public Action Listener_CallVote(int client, const char[] command, int args)
{
	...
	if (g_Changeplayerkick != 1 && g_Changeplayerkick == 0)
	{
		...
		{
			PrintToChat(client,"\x04[提示]\x05游戏自带的投票踢出玩家已禁用.请使用!votes投票");//聊天窗提示.
	...
	if (g_Changealltalk != 1 && g_Changealltalk == 0)
	{
		...
		{
			PrintToChat(client,"\x04[提示]\x05游戏自带的全局通话投票已禁用.请使用!votes投票");//聊天窗提示.
			return Plugin_Handled;
		}
	}
	if (g_Changerestartgame != 1 && g_Changerestartgame == 0)
	{
		...
		{
			PrintToChat(client,"\x04[提示]\x05游戏自带的投票重新开始已禁用.请使用!votes投票");//聊天窗提示.
			return Plugin_Handled;
		}
	}
	if (g_Changemission != 1 && g_Changemission == 0)
	{
		...
		{
			PrintToChat(client,"\x04[提示]\x05游戏自带的投票开始新图已禁用.请使用!votes投票");//聊天窗提示.
			return Plugin_Handled;
		}
	}
	if (g_Changechapter != 1 && g_Changechapter == 0)
	{
		...
		{
			PrintToChat(client,"\x04[提示]\x05游戏自带的投票更换章节已禁用.请使用!votes投票");//聊天窗提示.
			return Plugin_Handled;
		}
	}
	if (g_Changedifficulty != 1 && g_Changedifficulty == 0)
	{
		...
		{
			PrintToChat(client,"\x04[提示]\x05游戏自带的投票更改难度已禁用.请使用!votes投票");//聊天窗提示.
			return Plugin_Handled;
		}
	}
	return Plugin_Continue;
