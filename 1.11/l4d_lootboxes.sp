...
public void OnPluginStart()
{
	...
	g_hAllow =			CreateConVar("l4d_lootbox_enable",					"1",				"1 = 禁用插件 0 = 禁用插件", FCVAR_FLAGS, true, 0.0, true, 1.0);
	g_hGameModes =		CreateConVar("l4d_lootbox_gamemodes",				"",					"在这些游戏模式中启用插件，用逗号分隔，没有空格\n无内容则插件在全部游戏模式中生效", FCVAR_FLAGS);

	g_hBoxLifeTime =	CreateConVar("l4d_lootbox_lifetime",				"30.0",				"战利品箱子生成后多少秒消失", FCVAR_FLAGS, true, 10.0, true, 60.0);
	g_hPosProb =		CreateConVar("l4d_lootbox_positive_chance",			"50.0",				"战利品箱子开启的概率", FCVAR_FLAGS, true, 0.0, true, 100.0);

	g_hBoostTimes =		CreateConVar("l4d_lootbox_boost_durations",			BOOST_TIMES,		"战利品箱子得到的buff持续时间.\n 6个数值，用逗号分割.没有空格\n<加速,无敌,恢复血量,附带火焰攻击,无限子弹,附带爆炸射击>", FCVAR_FLAGS);
	g_hNerfTimes =		CreateConVar("l4d_lootbox_nerf_durations",			NERF_TIMES,			"战利品箱子得到的debuff持续时间.\n 1|3个数值，用逗号分割.没有空格\n<操作反转,受伤加倍,随机视角>\n如果只设置了一个数值\n则此数值将被设置为全部debuff的持续时间", FCVAR_FLAGS);
	if( g_bL4D2 )
	{
		g_hPosWeight =		CreateConVar("l4d_lootbox_positive_weights",		POS_WEIGHTS,		"战利品箱子得到的物品的概率\n 17个数值，用逗号分割.没有空格\n<请查看源码说明>", FCVAR_FLAGS);
		//T1武器,T2武器,T3武器,副武器,小药,大药,投抛,物品,增强弹药,激光瞄准,加速,无敌,恢复血量,附带火焰攻击,无限子弹,附带爆炸射击,处死全部小僵尸及全部特感血量减半
		//建议参考插件发布页的说明
		g_hNegRes =			CreateConVar("l4d_lootbox_negative_weights",		NEG_WEIGHTS,		"战利品箱子得到的debuff的概率\n17个数值，用逗号分割.没有空格\n<请查看源码说明>", FCVAR_FLAGS);
		//原地生成僵尸,召唤尸潮,胆汁爆炸糊附近生还者,生成多个酸液覆盖附近的确,原地生成Witch,生成Tank,Smoker毒气,原地生成Jockey,原地生成爆炸桶并立刻爆炸,生还者立刻黑白,冻结附近生还者,反向操作,受伤加倍,流血陷阱,视角随机,生成多个烟花并立刻燃烧,生成全部不重复的特感
		//建议参考插件发布页的说明
		g_hSpecialDrops =	CreateConVar("l4d_lootbox_special_drop_chance",		SI_CHANCES,			"设置各个特感掉落战利品箱子的概率.\n1|6个数值，用逗号分割.没有空格, 数值设定在0-100之间\n特感列表:<smoker,boomer,hunter,spitter,jockey,charger>\n如果只设置了一个数值\n则此数值将被设置为全部特感的掉落概率.", FCVAR_FLAGS);	
	}
	else
	{
		g_hPosWeight =		CreateConVar("l4d_lootbox_positive_weights",		POS_WEIGHTS_1,		"战利品箱子得到的物品的概率.\n14个数值，用逗号分割.没有空格\n<请查看源码说明>", FCVAR_FLAGS);
		//T1武器,T2武器,副武器,小药,大药,投抛,物品,加速,无敌,无敌,恢复血量,附带火焰攻击,无限子弹,附带爆炸射击,处死全部小僵尸及全部特感血量减半
		//建议参考插件发布页的说明
		g_hNegRes =			CreateConVar("l4d_lootbox_negative_weights",		NEG_WEIGHTS_1,		"战利品箱子得到的debuff的概率\n14个数值，用逗号分割.没有空格\n<请查看源码说明>", FCVAR_FLAGS);
		//原地生成僵尸,召唤尸潮,胆汁爆炸糊附近生还者,原地生成Witch,生成Tank,Smoker毒气,原地生成爆炸桶并立刻爆炸,生还者立刻黑白,冻结附近生还者,反向操作,受伤加倍,流血陷阱,视角随机,生成多个烟花并立刻燃烧,生成全部不重复的特感
		//建议参考插件发布页的说明
		g_hSpecialDrops =	CreateConVar("l4d_lootbox_special_drop_chance",		SI_CHANCES_1,		"设置各个特感掉落战利品箱子的概率\n1|3数值，用逗号分割.没有空格, 数值设定在0-100之间\n特感列表:<smoker,boomer,hunter>\n如果只设置了一个数值\n则此数值将被设置为全部特感的掉落概率," FCVAR_FLAGS);
	}
	g_hTankDrops =		CreateConVar("l4d_lootbox_tank_drops",				"1,3",				"坦克死亡时掉落的战利品的最小和最大数量\n1|2个数值，用逗号分割.没有空格\n如果只设置了一个数值\n则此数值将被设置为固定的战利品箱子掉落数量", FCVAR_FLAGS);
	g_hWitchDrops =		CreateConVar("l4d_lootbox_witch_drops",				"1,2",				"Witch死亡时掉落的战利品的最小和最大数量\n1|2个数值，用逗号分割.没有空格\n如果只设置了一个数值\n则此数值将被设置为固定的战利品箱子掉落数量", FCVAR_FLAGS);

	g_hWeaponLock =		CreateConVar("l4d_lootbox_weapon_lock",				"5.0",				"防止机器人偷窃武器/物品的时间 (0.0=禁用).", FCVAR_FLAGS, true, 0.0, true, 15.0);
	g_hWeaponLife =		CreateConVar("l4d_lootbox_weapon_lifetime",			"20.0",				"多少秒后箱子生成的武器/物品消失.", FCVAR_FLAGS, true, 15.0, true, 60.0);

	g_hToxCloudLife =	CreateConVar("l4d_lootbox_toxicloud_lifetime",		"40",				"毒烟的持续时间.", FCVAR_FLAGS, true, 10.0, true, 240.0);
	g_hIntoxChance = 	CreateConVar("l4d_lootbox_intoxication_chance",		"15.0",				"生还者承受毒烟伤害时，有多少几率中毒", FCVAR_FLAGS, true, 0.0, true, 100.0);
	g_hToxicHits =		CreateConVar("l4d_lootbox_toxichits",				"50",				"中毒的生还者将会受到多少毒素伤害", FCVAR_FLAGS, true, 1.0);
	g_hBleedHits =		CreateConVar("l4d_lootbox_bleedhits",				"30",				"生还者将因为流血而失去多少血量", FCVAR_FLAGS, true, 1.0);
	g_hFreezeTime =		CreateConVar("l4d_lootbox_freezetime",				"10.0",				"生还者将会被冻结多少秒", FCVAR_FLAGS, true, 1.0);
	g_hFragilityMult =	CreateConVar("l4d_lootbox_fragility_multiplier",	"5.0",				"生还者将会受到多少倍数的伤害", FCVAR_FLAGS, true, 1.0);

	g_hMobSize =		CreateConVar("l4d_lootbox_mob_size",				"30",				"战利品箱子最多生成多少僵尸", FCVAR_FLAGS, true, 20.0);
	g_hMegaMobSize =	CreateConVar("l4d_lootbox_megamob_size",			"30",				"战利品箱子最多在尸潮生成多少僵尸", FCVAR_FLAGS, true, 30.0);
...
Action AdminSpawnBox(int client, int args)		// Admin can spawn lootboxes at player position DISABLED
{
	...
	{
		PrintToChat(client, "%s 无法在此生成战利品盒子，请再次尝试", CHAT_TAG);
		return Plugin_Handled;
	}
	vPos[2] += 8.0;
	int iCount;
...
Action PlayerSpeed_Timer(Handle timer, int client)
{
	...
	PrintToChat(client, "%s \x03加速\x01效果已结束", CHAT_TAG);
	return Plugin_Continue;
}
...
Action PlayerInvul_Timer(Handle timer, int client)
{
	...
	EmitSoundToClient(client, SND_BOOST_END);
	PrintToChat(client, "%s \x03无敌\x01效果已结束", CHAT_TAG);
	return Plugin_Continue;
}
...
Action PlayerRegen_Timer(Handle timer, int client)
{
	...
	{
		PrintToChat(client, "%s \x03回血\x01效果已结束.", CHAT_TAG);
...
Action PlayerReverse_Timer(Handle timer, int client)
{
	...
	PrintToChat(client, "%s \x03反向操作\x01效果已结束", CHAT_TAG);
	EmitSoundToClient(client, SND_BOOST_END);
	return Plugin_Continue;
}
...
Action PlayerExpl_Timer(Handle timer, int client)
{
	...
	PrintToChat(client, "%s \x03爆炸射击\x01效果已结束", CHAT_TAG);
	EmitSoundToClient(client, SND_BOOST_END);
	return Plugin_Continue;
}
...
Action PlayerFragile_Timer(Handle timer, int client)
{
	...
	PrintToChat(client, "%s \x03捕猎陷阱\x01效果已结束", CHAT_TAG);
	EmitSoundToClient(client, SND_BOOST_END);
	return Plugin_Continue;
}
...
Action PlayerAngles_Timer(Handle timer, int client)
{
	...
	if( g_iPlayerBoosts[client] & PN_ANGLES )
	{
		...
		if( fTime <= 5.0 )
		{
			...
			PrintToChat(client, "%s \x03屏幕干扰\x01效果已结束", CHAT_TAG);
			EmitSoundToClient(client, SND_BOOST_END);	
		}
		else
		{
...
Action LootBox_Used(int entity, int caller, int activator, UseType type, float value)
{
	...
	if( g_fPlayerUse[activator] + 3.0 > GetGameTime() )
	{
		...
		{
			PrintToChat(activator, "%s 请等待 %.1f 秒后再打开战利品箱子", CHAT_TAG, 3 - GetGameTime() + g_fPlayerUse[activator]);
...
bool SPawnT1(int client, float vPos[3])
{
	...
	PrintToChat(client, "%s 你找到了一把 \x03小枪\x01！", CHAT_TAG);
...
bool SpawnT2(int client, float vPos[3])
{
	...
	TeleportEntity(iWeapon, vPos, NULL_VECTOR, NULL_VECTOR);
	PrintToChat(client, "%s 你找到了一把\x03大枪\x01！", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnT3(int client, float vPos[3])
{
	...
	if( GetRandomInt(0, 1) == 0 )
	{
		...
		TeleportEntity(iWeapon, vPos, NULL_VECTOR, NULL_VECTOR);
		PrintToChat(client, "%s 你找到了一把\x03M60重机枪\x01！", CHAT_TAG);
	}
	else
	{
		...
		TeleportEntity(iWeapon, vPos, NULL_VECTOR, NULL_VECTOR);
		PrintToChat(client, "%s 你找到了一把\x03榴弹发射器\x01！", CHAT_TAG);
	}
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnSecondary(int client, float vPos[3])
{
	...
	PrintToChat(client, "%s 你找到了一把\x03副武器\x01！", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnDrugs(int client, float vPos[3])
{
	...
	PrintToChat(client, "%s 你找到了\x03小药\x01！", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnMeds(int client, float vPos[3])
{
	...
	PrintToChat(client, "%s 你找到了\x03医疗物品\x01！", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnThrowable(int client, float vPos[3])
{
	...
	TeleportEntity(iWeapon, vPos, NULL_VECTOR, NULL_VECTOR);
	PrintToChat(client, "%s 你找到了一个\x03投抛物品\x01！", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnItem(int client, float vPos[3])
{
	...
	if( iEntity != -1 )
	{
		...
		PrintToChat(client, "%s 你找到了一个\x03%s\x01！", CHAT_TAG, sName);
		return true;
	}
	return false;
}
...
bool SpawnUpgrade(int client, float vPos[3])
{
	...
	TeleportEntity(iWeapon, vPos, NULL_VECTOR, NULL_VECTOR);
	PrintToChat(client, "%s 你找到了一个\x03升级包\x01！", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnLaser(int client, float vPos[3])
{
	...
	for( int i = 1; i <= MaxClients; i++ )
	{
		if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
			PrintToChat(i, "%s \x03%N\x01找到了一个\x03激光瞄准器\x01！", CHAT_TAG, client);
	}
	PrintToChat(client, "%s 你找到了一个\x03激光瞄准器\x01！", CHAT_TAG);
	return true;
}
...
void GivePlayerSpeed(int client)
{
	if( CheckPlayerBoost(client, PB_SPEED) )
	{
		...
		PrintToChat(client, "%s 你已经延长了你的\x03加速\x01buff.", CHAT_TAG);
		return;
	}
	...
	g_hPlayerSpeedTimer[client] = CreateTimer(g_fBoostTimes[0], PlayerSpeed_Timer, client);
	g_iPlayerBoosts[client] |= PB_SPEED;
	PrintToChat(client, "%s 你获得了\x03加速\x01buff", CHAT_TAG);
...
void GivePlayerInvulnerability(int client)
{
	if( CheckPlayerBoost(client, PB_INVUL) )
	{
		...
		PrintToChat(client, "%s 你已经延长了你的\x03无敌\x01buff", CHAT_TAG);
		return;
	}
	...
	g_hPlayerInvulTimer[client] = CreateTimer(g_fBoostTimes[1], PlayerInvul_Timer, client);
	g_iPlayerBoosts[client] |= PB_INVUL;
	PrintToChat(client, "%s 你获得了\x03无敌\x01buff\n你被强化了！快去送吧", CHAT_TAG);
}
...
void GivePlayerRegen(int client)
{
	...
	{
		PrintToChat(client, "%s 你已经延长了你的\x03血量恢复\x01buff", CHAT_TAG);
		return;		
	}
	...
	g_iPlayerBoosts[client] = PB_REGEN;
	PrintToChat(client, "%s 你获得了\x03血量恢复\x01buff", CHAT_TAG);
...
void GivePlayerFire(int client)
{
	if( CheckPlayerBoost(client, PB_FIRE) )
	{
		...
		PrintToChat(client, "%s 你已经延长了你的\x03火焰射击\x01buff", CHAT_TAG);
		return;
	}
	...
	g_hPlayerFireTimer[client] = CreateTimer(g_fBoostTimes[3], PlayerFire_Timer, client);
	g_iPlayerBoosts[client] = PB_FIRE;
	PrintToChat(client, "%s 你获得了\x01火焰射击\x01buff", CHAT_TAG);
...
void GivePlayerInfAmmo(int client)
{
	if( CheckPlayerBoost(client, PB_IAMMO) )
	{
		...
		PrintToChat(client, "%s 你已经延长了你的\x03无限子弹\x01buff", CHAT_TAG);
		return;
	}
	
	g_hPlayerAmmoTimer[client] = CreateTimer(g_fBoostTimes[4], PlayerInfAmmo_Timer, client);
	g_iPlayerBoosts[client] = PB_IAMMO;
	PrintToChat(client, "%s 你获得了\x03无限子弹\x01buff", CHAT_TAG);
}
...
void GivePlayerExplosive(int client)
{
	if( CheckPlayerBoost(client, PB_EXPL) )
	{
		...
		PrintToChat(client, "%s 你已经延长了你的\x03爆炸射击\x01buff", CHAT_TAG);
	}
	else
	{
		g_iPlayerBoosts[client] |= PB_EXPL;
		g_hPlayerExplTimer[client] = CreateTimer(g_fBoostTimes[5], PlayerExpl_Timer, client);
		PrintToChat(client, "%s 你获得了\x03爆炸射击\x01buff", CHAT_TAG);
	}
}
...
void WipeHalfZombies(int client)
{
	...
	for( int i = 1; i <= MaxClients; i++ )
	{
		...
		if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
		{
			PrintToChat(i, "%s \x03%N\x01获得了一个\x03nb的道具\x01，场上所有的小僵尸已经没了，全部特感的血量减半", CHAT_TAG, client);
		}
	}
	EmitSoundToAll(SND_THANOS);
	PrintToChat(client, "%s 你找到了一个\x03nb的道具\x01，场上所有的小僵尸已经没了，全部特感的血量减半", CHAT_TAG);
}
...
void SpawnMob(int client, bool mega)
{
	if( mega )
	{
		...
		{
			if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
				PrintToChat(i, "%s \x03%N\x01因打开战利品箱子召唤了\x03尸潮\x01.", CHAT_TAG, client);
		}
		PrintToChat(client, "%s 你召唤了\x03尸潮\x01.", CHAT_TAG);
	...
	else
	{
		...
		{
			if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
				PrintToChat(i, "%s \x03%N\x01因打开战利品箱子生成了\x03一些小僵尸\x01.", CHAT_TAG, client);
		}
		PrintToChat(client, "%s 你因打开战利品箱子生成了\x03一些小僵尸\x01.", CHAT_TAG);
	}
...
void BoxTrap(int client, float vPos[3], bool isVomit)
{
	...
	for( int i = 1; i <= MaxClients; i++ )
	{
		...
		if( client != i && !IsFakeClient(i) )
			PrintToChat(i, "%s \x03%N\x01因打开战利品箱子得到了 %s 的debuff", CHAT_TAG, client, isVomit ? "\x03糊附近玩家的脸\x01." : "\x03冻结附近玩家\x01.");
	...
	PrintToChat(client, "%s 你因打开战利品箱子得到了 %s 的debuff", CHAT_TAG, isVomit ? "\x03糊附近玩家的脸\x01" : "\x03冻结附近玩家\x01");
}
...
void ReverseControls(int client)
{
	if( g_iPlayerBoosts[client] & PN_REVERSE)
	{
		...
		PrintToChat(client, "%s 你已经延长了你的\x03反向操作\x01debuff", CHAT_TAG);
		return;
	}
	g_hPlayerReverseTimer[client] = CreateTimer(g_fNerfTimes[0], PlayerReverse_Timer, client);
	PrintToChat(client, "%s 你获得了\x03反向操作\x01debuff", CHAT_TAG);
...
void SpitTrap(int client, float vPos[3])
{
	...
	{
		if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
			PrintToChat(i, "%s \x03%N\x01 因为打开箱子而生成了\x03一堆酸液\x01", CHAT_TAG, client);
	}
	PrintToChat(client, "%s 你因打开箱子而生成了\x03一堆酸液\x01", CHAT_TAG);
}
...
bool SpawnWitch(int client, float vPos[3])
{
	...
	for( int i = 1; i <= MaxClients; i++ )
	{
		if( IsClientInGame(i) && i != client && !IsFakeClient(i) )
			PrintToChat(i, "%s \x03%N\x01找到了一个\x03Witch\x01", CHAT_TAG, client);
	}
	PrintToChat(client, "%s 你找到了一个\x03Witch\x01", CHAT_TAG);
	return true;
}
...
bool SpawnTank(int client, float vPos[3])
{
	...
	{
		if( IsClientInGame(i) && i != client && !IsFakeClient(i) )
			PrintToChat(i, "%s \x03%N\x01因为打开箱子而召唤了\x03Tank\x01", CHAT_TAG, client);
	}
	PrintToChat(client, "%s 你因打开箱子而召唤了\x03Tank\x01", CHAT_TAG);
	return true;
}
...
bool ToxicCloud(int client, float vPos[3])
{
	...
	{
		if( IsClientInGame(i) && i != client && !IsFakeClient(i) )
			PrintToChat(i, "%s \x03%N\x01因为打开箱子而放出了\x03毒烟\x01", CHAT_TAG, client);
	}
	...
	PrintToChat(client, "%s 你因打开箱子而放出了\x03毒烟\x01", CHAT_TAG);
	return true;
}
...
bool JockeyRide(int client)
{
	...
	if( result > 0 )
	{
		PrintToChat(client, "%s 某人打开箱子而召唤了一个\x03Jockey\x01", CHAT_TAG);
		return true;
	}
	return false;
}
...
bool SpawnExplBarrel(int client, float vPos[3])
{
	...
	{
		if( IsClientInGame(i) && i != client && !IsFakeClient(i) )
			PrintToChat(i, "%s \x03%N\x01获得了一个\x03爆炸了的油桶\x01", CHAT_TAG, client);
	}
	PrintToChat(client, "%s 你获得了一个\x03爆炸了的油桶\x01", CHAT_TAG);
	return true;
}
...
void BlackWhite(int client)
{
	...
	PrintToChat(client, "%s 你因开启战利品箱子获得了\x03黑白\x01debuff", CHAT_TAG);
} 
...
void GiveFragility(int client)
{
	...
	if( g_iPlayerBoosts[client] & PB_INVUL )
	{
		...
		SDKUnhook(client, SDKHook_OnTakeDamage, Invulnerability_Callback);
		EmitSoundToClient(client, SND_BAD_OPEN);
		PrintToChat(client, "%s 你因开启战利品箱子获得了\x03脆弱\x01debuff, 你的\x03恢复健康\x01buff已经移除", CHAT_TAG);
	}
	else if( g_iPlayerBoosts[client] & PN_FRAGILE )
	{
		...
		EmitSoundToClient(client, SND_BAD_OPEN);
		PrintToChat(client, "%s 你已经延长了你的\x03脆弱\x01debuff", CHAT_TAG);
	}
	else
	{
		g_iPlayerBoosts[client] |= PN_FRAGILE;
		...
		EmitSoundToClient(client, SND_BAD_OPEN);
		PrintToChat(client, "%s 你因开启战利品箱子获得了\x03脆弱\x01debuff", CHAT_TAG);
	}
}

void BearTrap(int client, float vPos[3])
{
	PrintToChat(client, "%s 你因开启战利品箱子获得了\x03捕猎陷阱\x01debuff", CHAT_TAG);
...
void RandomAngles(int client)
{
	...
	if( g_iPlayerBoosts[client] & PN_ANGLES )
	{
		PrintToChat(client, "%s 你已经延长了你的\x03随机视角\x01debuff", CHAT_TAG);
		return;
	}
	g_iPlayerBoosts[client] |= PN_ANGLES;
	PrintToChat(client, "%s 你获得了\x03随机视角\x01debuff",CHAT_TAG);
...
bool SpawnFireworks(int client, float vPos[3])
{
	...
	if( result )
	{
		...
		{
			if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
				PrintToChat(i, "%s \x03%N\x01获得了一个\x03烟花排队\x01", CHAT_TAG, client);
		}
		PrintToChat(client, "%s 你发现了一个\x03烟花排队\x01", CHAT_TAG);
	}
	return result;
}
...
bool SpawnFullTeam(int client)
{
	...
	{
		if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
			PrintToChat(i, "%s \x03%N\x01让特感们来查水表了！", CHAT_TAG, client);
	}
	PrintToChat(client, "%s 你让特感们来查水表了！", CHAT_TAG);
	return result;
}
