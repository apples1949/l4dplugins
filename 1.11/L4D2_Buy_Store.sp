...
public void OnPluginStart()
{
	...
	//  ConVar //
	//****************//
	g_PlayerRequired = CreateConVar("sm_shop_player_require", "4", "多少真实的感染者和生还者才能激活插件", FCVAR_NOTIFY, true, 1.0);
	...
	g_BoomerKilled = CreateConVar("sm_shop_boomkilled", "10", "击杀boomer能得到多少金钱", FCVAR_NOTIFY, true, 1.0);
	g_ChargerKilled = CreateConVar("sm_shop_chargerkilled", "30", "击杀charger能得到多少金钱", FCVAR_NOTIFY, true, 1.0);
	g_SmokerKilled = CreateConVar("sm_shop_smokerkilled", "20", "击杀smoker能得到多少金钱", FCVAR_NOTIFY, true, 1.0);
	g_HunterKilled = CreateConVar("sm_shop_hunterkilled", "20", "击杀hunter能得到多少金钱", FCVAR_NOTIFY, true, 1.0);
	g_JockeyKilled = CreateConVar("sm_shop_jockeykilled", "25", "击杀jockey能得到多少金钱", FCVAR_NOTIFY, true, 1.0);
	g_SpitterKilled = CreateConVar("sm_shop_spitterkilled", "10", "击杀spitter能得到多少金钱", FCVAR_NOTIFY, true, 1.0);
	g_hTankHurt = CreateConVar("sm_shop_tank_hurt", "40", "给坦克造成多少伤害才能得到1金钱", FCVAR_NOTIFY, true, 1.0);
	g_WitchKilled = CreateConVar("sm_shop_witchkilled", "80", "击杀witch能得到多少金钱", FCVAR_NOTIFY, true, 1.0);
	g_ZombieKilled = CreateConVar("sm_shop_zombiekilled", "1", "击杀僵尸能得到多少金钱", FCVAR_NOTIFY, true, 1.0);
	g_hHealTeammate = CreateConVar("sm_shop_heal_teammate", "100", "用医疗包治疗队友给多少钱", FCVAR_NOTIFY, true, 1.0);
	g_hDefiSave = CreateConVar("sm_shop_defi_save", "200", "电击队友复活奖励多少钱", FCVAR_NOTIFY, true, 1.0);
	g_hHelpTeammate = CreateConVar("sm_shop_help_teammate_save", "30", "救下队友的奖励. (No Hanging from legde)", FCVAR_NOTIFY, true, 1.0);
	g_hIncapSurvivor = CreateConVar("sm_shop_infected_survivor_incap", "30", "感染者玩家击倒幸存者给多少钱. (No Hanging from legde)", FCVAR_NOTIFY, true, 1.0);
	g_hKillSurvivor = CreateConVar("sm_shop_infected_survivor_killed", "100", "击杀生还者能得到多少金钱.", FCVAR_NOTIFY, true, 1.0);
	g_hTKSurvivorEnable = CreateConVar("sm_shop_survivor_TK_enable", "1", "如果为1，友伤扣钱，黑1血扣多少钱，如果为0则不扣钱. ", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hGascanMapOff = CreateConVar("sm_shop_gascan_map_off",	"c1m4_atrium,c6m3_port,c14m2_lighthouse",	"这些地图用不能购买油桶, 用逗号分割(无空格). (0=全部的地图, 空格 = 无).", FCVAR_NOTIFY );
	g_hColaMapOff =	CreateConVar("sm_shop_cola_map_off",	"c1m2_streets",	"这些地图用不能购买可乐瓶, 用逗号分割(无空格). (0=全部的地图, 空格 = 无).", FCVAR_NOTIFY );
	g_hStageComplete =	CreateConVar("sm_shop_stage_complete", "400",	"过关给生还者多少钱(非救援).", FCVAR_NOTIFY, true, 1.0);
	...
	g_hWipeOutSurvivor =	CreateConVar("sm_shop_final_mission_lost", "300",	"团灭生还者扣多少钱以及有感染者玩家灭团生还者给多少钱.", FCVAR_NOTIFY, true, 1.0);
	g_hInfectedShopEnable =	CreateConVar("sm_shop_infected_enable", "1",	"如果为1，则启动感染者购买菜单.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hInfectedShopTime = CreateConVar("sm_shop_infected_wait_time", "10", "感染者玩家必须等到幸存者离开安全区至少多少秒后才能购买物品。(0=感染者的商店随时可用)", FCVAR_NOTIFY, true, 0.0);
	g_hInfectedShopColdDown =	CreateConVar("sm_shop_infected_cooltime_block", "30.0",	"感染者购买冷却时间(0=无).", FCVAR_NOTIFY, true, 0.0);
	g_hSurvivorShopColdDown =	CreateConVar("sm_shop_survivor_cooltime_block", "5.0",	"C在玩家购买物品后，多长时间才能再次购买（秒）(0=关闭)", FCVAR_NOTIFY, true, 0.0);
	...
	g_hNotifyKillInfectedType = CreateConVar("sm_shop_kill_infected_announce_type",	"1", "改变击杀敌人给予金钱信息的显示方式(0: 不显示, 1:在聊天框中, 2: 在提示框中, 3: 在屏幕中心？)", FCVAR_NOTIFY, true, 0.0, true, 3.0);
