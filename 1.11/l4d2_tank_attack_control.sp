...
public void TankSpawn_Event(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		C_PrintToChat(tank, "{blue}[{default}Tank Rock Selector{blue}]");
		C_PrintToChat(tank, "{olive}Reload {default}= {blue}2 Handed Overhand");
		C_PrintToChat(tank, "{olive}Use {default}= {blue}Underhand");
		C_PrintToChat(tank, "{olive}M2 {default}= {blue}1 Handed Overhand");
	}
}
