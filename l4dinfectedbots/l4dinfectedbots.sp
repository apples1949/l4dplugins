...
public void OnPluginStart()
{
	...
	// console variables
	g_hCvarAllow =		CreateConVar(	"l4d_infectedbots_allow",			"1",			"0=关闭插件, 1=开启插件", FCVAR_NOTIFY );
	...
	h_PlayerAddTankLimit = CreateConVar("l4d_infectedbots_add_tanklimit", "1", "玩家数量超过5个时，Tank数量上限 = tank_limit + [(存活的倖存者数量-4) ÷ 'add_tanklimit_scale'] × 'add_tanklimit'.", FCVAR_NOTIFY, true, 0.0);
	...
	h_PlayerAddZombiesScale = CreateConVar("l4d_infectedbots_add_specials_scale", "2", "玩家数量超过5个时，最大特感数量上限 = max_specials + [(存活的生还者数量-4) ÷ 'add_specials_scale'] × 'add_specials'.", FCVAR_NOTIFY, true, 1.0);
	h_PlayerAddZombies = CreateConVar("l4d_infectedbots_add_specials", "2", "玩家数量超过5个时，最大特感数量上限 = max_specials + [(存活的生还者数量-4) ÷ 'add_specials_scale'] × 'add_specials'", FCVAR_NOTIFY, true, 0.0);
	...
	h_PlayerAddTankHealthScale = CreateConVar("l4d_infectedbots_add_tankhealth_scale", "1", "玩家数量超过5个时，坦克血量上限 = max_specials + [(存活的生还者数量-4) ÷ 'add_specials_scale'] × 'add_specials'].", FCVAR_NOTIFY, true, 1.0);
	...
	if (!g_bL4D2Version)
	{
		h_StatsBoard = CreateConVar("l4d_infectedbots_stats_board", "0", "如果为1，则感染者玩家死亡后可开放统计界面（仅1代）", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	}
	h_JoinableTeamsAnnounce = CreateConVar("l4d_infectedbots_coop_versus_announce", "1", "如果为1，则通知玩家如何加入到生还者和感染者", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	h_Coordination = CreateConVar("l4d_infectedbots_coordination", "0", "如果为1，则感染者玩家需要全部复活才能一起攻击", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	h_InfHUD = CreateConVar("l4d_infectedbots_infhud_enable", "1", "感染者玩家是否开启HUD.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	h_Announce = CreateConVar("l4d_infectedbots_infhud_announce", "1", "是否提示感染者玩家如何开启HUD", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	h_Idletime_b4slay = CreateConVar("l4d_infectedbots_lifespan", "30", "AI特感生成多少秒后踢出（AI防卡）", FCVAR_NOTIFY, true, 1.0);
	...
	h_SpawnDistanceMax = CreateConVar("l4d_infectedbots_spawn_range_max", "2000", "生成特感的最大范围（默认1500，仅战役/写实模式）", FCVAR_NOTIFY, true, 1.0);
	h_SpawnDistanceFinal = CreateConVar("l4d_infectedbots_spawn_range_final", "0", "救援开始时生成特感的范围（仅战役/写实模式）", FCVAR_NOTIFY, true, 0.0);
	...
	h_ZSDisableGamemode = CreateConVar("l4d_infectedbots_sm_zs_disable_gamemode", "6", "在哪些游戏模式中禁止感染者玩家使用sm_zs (0: 无, 1: 战役/写实, 2: 对抗/清道夫, 4: 生还者, 多个模式添加数字输出)", FCVAR_NOTIFY, true, 0.0, true, 7.0);
	...
	h_PlayerAddCommonLimitScale = CreateConVar("l4d_infectedbots_add_commonlimit_scale", "1", "超过五人以上玩家，最大普通僵尸数量上限 = default_commonlimit + [(存活的生还者数量-4) ÷ 'add_commonlimit_scale'] × 'add_commonlimit'", FCVAR_NOTIFY, true, 1.0);
	h_PlayerAddCommonLimit = CreateConVar("l4d_infectedbots_add_commonlimit", "2", "超过五人以上玩家，最大普通僵尸数量上限 = default_commonlimit + [(存活的生还者数量-4) ÷ 'add_commonlimit_scale'] × 'add_commonlimit'", FCVAR_NOTIFY, true, 0.0);
	h_CoopInfectedPlayerFlashLight = CreateConVar("l4d_infectedbots_coop_versus_human_light", "1", "如果为1，则特感玩家将发出红色的光", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	h_StatusAnnouncementEnable = CreateConVar("l4d_infectedbots_announcement_enable", "1", "如果为1，则当生还者数量发生变化时宣布插件状态", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	h_CoopInfectedPlayerGhostState = CreateConVar("l4d_infectedbots_coop_versus_human_ghost_enable", "1", "如果为1，则在战役/写实/生还者模式中，感染者玩家将以灵魂状态复活", FCVAR_NOTIFY, true, 0.0, true, 1.0);
...
public Action CheckGameMode(int client, int args)
{
	if (client)
	{
		PrintToChat(client, "g_iCurrentMode = %i", g_iCurrentMode);
	}
...
public Action CheckQueue(int client, int args)
{
	...
	if (client)
	{
		...
		PrintToChat(client, "InfectedBotQueue = %i, InfectedBotCount = %i, InfectedRealCount = %i, InfectedRealQueue = %i", InfectedBotQueue, InfectedBotCount, InfectedRealCount, InfectedRealQueue);
	}
...
public Action Console_ZLimit(int client, int args)
{
	...
	if(IsInteger(arg1))
	{
		...
		else if(newlimit!=g_iMaxPlayerZombies)
		{
			...
			CreateTimer(0.1, MaxSpecialsSet);
			C_PrintToChatAll("{lightgreen}%N{default}: %t", client, "Special Infected Limit has been changed",newlimit);
	...
	if(args == 1)
	{
		...
		if(IsInteger(arg1))
		{
			...
			else
			{
				...
				C_PrintToChatAll("{lightgreen}%N{default}: %t",client,"Bot Spawn Timer has been changed",DD,DD);
			}
			return Plugin_Handled;
		}
		else
	...
	else
	{
		...
		if(IsInteger(arg1) && IsInteger(arg2))
		{
			...
			else
			{
				...
				C_PrintToChatAll("{lightgreen}%N{default}: %t",client,"Bot Spawn Timer has been changed",Min,Max);
			}
			return Plugin_Handled;
		}
		else
	...
	if(iAliveSurplayers >= 0 && (iAliveSurplayers != iPlayersInSurvivorTeam))
	{
		...
		if(g_bTankHealthAdjust)
		{
			...
			if(g_bCommonLimitAdjust)
			{
				...
				if(g_bStatusAnnouncementEnable) C_PrintToChatAll("%t","Current status1",iAliveSurplayers,g_iMaxPlayerZombies,zombieHP[6],h_common_limit_cvar.IntValue);
			}
			else
			{
				if(g_bStatusAnnouncementEnable) C_PrintToChatAll("%t","Current status3",iAliveSurplayers,g_iMaxPlayerZombies,zombieHP[6]);
			}
		}
		else
		{
			...
			{
				if(g_bStatusAnnouncementEnable) C_PrintToChatAll("%t","Current status4",iAliveSurplayers,g_iMaxPlayerZombies);
			}
		}
...
public Action TimerAnnounce2(Handle timer, int client)
{
	if (IsClientInGame(client))
	{
		...
		{
			C_PrintToChat(client, "%T","sm_zs",client);
		}
	}
